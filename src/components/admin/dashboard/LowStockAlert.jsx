  ...
  return (
    <div className="border border-border bg-muted p-6">
      ...
      <div className="flex flex-col gap-0">
        {products.length === 0 && (
          <p className="text-[12px] text-muted-foreground py-4">No low stock items right now.</p>
        )}
        {products.map((item) => (
          <div key={item._id} className="flex items-center justify-between py-3.5 border-b border-border last:border-0">
            ...
            <div className="text-right">
              <p className={`text-[18px] font-light leading-none ${item.totalStock <= 3 ? "text-[#f87171]" : "text-[#fbbf24]"}`} style={{ fontFamily: "var(--font-serif)" }}>
                {item.totalStock}
              </p>
              <p className="text-[9px] tracking-[0.1em] uppercase text-muted-foreground mt-0.5">units left</p>
            </div>
          </div>
        ))}
      </div>

      <Link href="/admin/products" className="block text-center text-[9px] font-bold tracking-[0.18em] uppercase text-muted-foreground hover:text-foreground transition-colors mt-5 pt-4 border-t border-border">
        Manage Inventory →
      </Link>
    </div>
  );
