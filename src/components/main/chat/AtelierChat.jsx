...
import { sendChatMessage } from "@/lib/actions/customer/chat";
...
export default function AtelierChat() {
  ...
  return (
    <>
      ...
      <div
        className={`fixed inset-y-0 right-0 z-50 w-full sm:w-[400px] bg-zinc-950 border-l border-zinc-900/80 shadow-2xl transform transition-transform duration-500 ease-out flex flex-col justify-between ${isOpen ? "translate-x-0" : "translate-x-full"
          }`}
      >
        <Card className="rounded-none border-0 ring-0 bg-transparent shadow-none flex flex-col h-full">
          ...
          >
            {messages.map((msg) => (
              <div
                key={msg.id}
                className={`flex flex-col space-y-2 text-left max-w-[85%] ${msg.role === "user" ? "ml-auto items-end" : "mr-auto items-start"
                  }`}
              >
                <div
                  className={`px-4 py-3 text-xs tracking-wide leading-relaxed font-sans ${msg.role === "user"
                  ...
                  <div className="w-full pt-2 grid grid-cols-1 gap-2">
                    {msg.products.map((prod) => (
                      ...
                      >
                        ...
                        <a href={`/collections/product/${prod.slug}`}
                          className="text-[9px] uppercase tracking-widest text-zinc-300 hover:text-zinc-100 border-b border-zinc-600 pb-0.5 transition-colors shrink-0"
                        >
                          View
                        </a>
